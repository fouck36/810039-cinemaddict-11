...
const createMostComentedFilmsTemplate = () => {
  return (
    `<section class="films-list--extra">
...
render(siteMainElement, craeteFilterTemplate(), `beforeend`);
...
for (let i = 0; i < ALL_FILMS_COUNT; i++) {
  render(AllfilmsContainer, createFilmCardTemplate(), `beforeend`);
};

render(AllFilmsList, createShowMoreButtonTemplate(), `beforeend`);

render(siteFilmsDesk, createTopRatedFilmsTemplate(), `beforeend`);
render(siteFilmsDesk, createMostComentedFilmsTemplate(), `beforeend`);
...
render(siteBodyElement, filmDetailsPopupTemplate(), `beforeend`);
